...
void main_game_loop(){
    ...
    // Loding the images
    SDL_Texture *Back_ground = (window->loadTexture("res/images/back.jpg"));
    Button *b = new Button(1100,620,100,100,"res/images/lead.png");
    Button *c = new Button(600,280,100,100,"res/images/play.png");
    ...
    {   
        ...
        if(Play_screen){
            ...
            /*for(int i=0;i<elist.size();i++) //check all the enemies.
            {
                if (hero.collision(elist[i],hero)==0 || hero.collision(elist[i],hero)==1)
                {
                    collide=true;
                    if(attack=true)
                    {
